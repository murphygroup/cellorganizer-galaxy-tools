...
for i = 1:30
    dna{i} = @() flipdim(OME_loadchannel([directory filesep files{i}],1),3);
    cell{i} = @() flipdim(OME_loadchannel([directory filesep files{i}],2),3);
    protein{i} = @() flipdim(OME_loadchannel([directory filesep files{i}],3),3);
end
